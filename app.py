...
# ==================================================================================================
# ALMOXARIFADO - CONTROLE DE ESTOQUE COM ENTRADA E SAÍDA
# ==================================================================================================
    ...
    # ======================
    def gerar_id_produto(produtos: List[ProdutoEstoque]) -> str:
        ...
        ids = []
        for p in produtos:
            if p.id and p.id.startswith("PROD-"):
                try:
    ...
    def gerar_id_saida(saidas: List[RegistroSaida]) -> str:
        ...
        ids = []
        for s in saidas:
            if s.id and s.id.startswith("SAI-"):
                try:
    ...
    @retry_on_quota()
    @st.cache_data(ttl=300)
    def carregar_produtos_estoque() -> List[ProdutoEstoque]:
        ...
        try:
            ...
            for row in todos_dados[1:]:
                if len(row) <= max([i for i in [idx_id, idx_categoria, idx_produto, idx_ca, idx_base, idx_quantidade] if i is not None]):
                    continue
                
                try:
                    produto = ProdutoEstoque()
                    produto.id = row[idx_id].strip() if idx_id is not None and len(row) > idx_id else ""
                    produto.categoria = row[idx_categoria].strip() if idx_categoria is not None and len(row) > idx_categoria else ""
                    produto.produto = row[idx_produto].strip() if idx_produto is not None and len(row) > idx_produto else ""
    ...
    @retry_on_quota()
    @st.cache_data(ttl=300)
    def carregar_saidas() -> List[RegistroSaida]:
        """Carrega os registros de saída da aba SAÍDA"""
        saidas = []
        try:
            client = get_gspread_client()
            if client is None:
            ...
            for row in todos_dados[1:]:
                try:
                    saida = RegistroSaida()
                    saida.id = row[idx_id].strip() if idx_id is not None and len(row) > idx_id else ""
                    ...
                    saida.produto = row[idx_produto].strip() if idx_produto is not None and len(row) > idx_produto else ""
                    saida.categoria = row[idx_categoria].strip() if idx_categoria is not None and len(row) > idx_categoria else ""
                    saida.colaborador = row[idx_colaborador].strip() if idx_colaborador is not None and len(row) > idx_colaborador else ""
                    
                    if idx_quantidade is not None and len(row) > idx_quantidade and row[idx_quantidade]:
                        try:
                            saida.quantidade = float(str(row[idx_quantidade]).replace(',', '.'))
                        except:
                            saida.quantidade = 0.0
                    
                    saida.obs = row[idx_obs].strip() if idx_obs is not None and len(row) > idx_obs else ""
                    saida.responsavel = row[idx_responsavel].strip() if idx_responsavel is not None and len(row) > idx_responsavel else ""
    ...
    # ======================
    def salvar_produto(produto: ProdutoEstoque) -> tuple:
        ...
        try:
            ...
            dados = [
                ...
                produto.categoria,
                produto.produto,
                str(produto.ca).replace('.', ','),
                str(produto.base).replace('.', ','),
                str(produto.quantidade).replace('.', ',')
            ]
            
            sheet.append_row(dados)
            st.cache_data.clear()
            return True, f"✅ Produto {produto.produto} salvo com sucesso!"
            
        except Exception as e:
            return False, f"❌ Erro ao salvar: {str(e)}"
    
    def atualizar_produto(produto: ProdutoEstoque) -> tuple:
        ...
        try:
            ...
            cell = sheet.find(produto.id, in_column=1)
            if not cell:
                return False, f"❌ Produto {produto.id} não encontrado"
            ...
            st.cache_data.clear()
            return True, f"✅ Produto {produto.produto} atualizado com sucesso!"
    ...
    # FUNÇÕES CRUD - SAÍDAS
    # ======================
    def salvar_saida(saida: RegistroSaida) -> tuple:
        ...
        try:
            ...
            dados_saida = [
                saida.id,
                saida.data.strftime("%d/%m/%Y") if saida.data else "",
                saida.produto,
                saida.categoria,
                saida.colaborador,
                str(saida.quantidade).replace('.', ','),
                saida.obs,
                saida.responsavel
            ]
            ...
            cell = sheet_base.find(saida.produto, in_column=3)  # Coluna PRODUTO
            if not cell:
                return False, f"❌ Produto {saida.produto} não encontrado no estoque"
            ...
            nova_qtd = qtd_atual - saida.quantidade
            
            if nova_qtd < 0:
                return False, f"❌ Estoque insuficiente! Disponível: {qtd_atual:.2f}, Solicitado: {saida.quantidade:.2f}"
            
            # Atualizar quantidade
            sheet_base.update_cell(cell.row, 6, str(nova_qtd).replace('.', ','))
            
            st.cache_data.clear()
            return True, f"✅ Saída {saida.id} registrada com sucesso! Novo estoque: {nova_qtd:.2f}"
    ...
    # ======================
    def gerar_relatorio_almoxarifado(produtos: List[ProdutoEstoque], saidas: List[RegistroSaida], 
        ...
        for p in produtos:
            ...
            categorias[p.categoria] += p.quantidade
        
        # Produtos com estoque baixo (menos de 5 unidades)
        estoque_baixo = [p for p in produtos if p.quantidade < 5 and p.quantidade > 0]
        estoque_zero = [p for p in produtos if p.quantidade <= 0]
            ...
            for p in sorted(produtos, key=lambda x: x.produto):
                ...
                html += f"""
                            <tr>
                                ...
                                <td>{p.ca:.2f}</td>
                                <td>{p.base:.2f}</td>
                                <td><strong>{p.quantidade:.2f}</strong></td>
                                <td>{status}</td>
                            </tr>
                """
        else:
        ...
        if saidas:
            ...
            for s in sorted(saidas, key=lambda x: x.data if x.data else datetime.min, reverse=True)[:50]:
                data_str = s.data.strftime("%d/%m/%Y") if s.data else "-"
                html += f"""
                            <tr>
                                <td>{s.id}</td>
                                <td>{data_str}</td>
                                ...
                                <td>{s.categoria}</td>
                                <td>{s.colaborador}</td>
                                <td><strong>{s.quantidade:.2f}</strong></td>
                                <td>{s.responsavel}</td>
                            </tr>
                """
    ...
    # ======================
    def gerar_relatorio_saidas_html(saidas: List[RegistroSaida], data_ini=None, data_fim=None) -> str:
        ...
        if data_ini:
            saidas_filtradas = [s for s in saidas_filtradas if s.data and s.data >= data_ini]
        if data_fim:
            saidas_filtradas = [s for s in saidas_filtradas if s.data and s.data <= data_fim]
        
        total_saidas = len(saidas_filtradas)
        total_qtd = sum(s.quantidade for s in saidas_filtradas)
        
        # Agrupar por produto
        produtos_saidas = {}
        for s in saidas_filtradas:
            if s.produto not in produtos_saidas:
                produtos_saidas[s.produto] = 0
            produtos_saidas[s.produto] += s.quantidade
            ...
            <div class="container">
                ...
                <div class="cards">
                    ...
                    <div class="card card-purple">
                        ...
                        <div class="value">{(total_qtd / total_saidas):.2f}" if total_saidas > 0 else "0.00"</div>
                    </div>
                </div>
        """
        ...
        if saidas_filtradas:
            ...
            for s in sorted(saidas_filtradas, key=lambda x: x.data if x.data else datetime.min, reverse=True):
                data_str = s.data.strftime("%d/%m/%Y") if s.data else "-"
                obs = s.obs[:50] + "..." if len(s.obs) > 50 else s.obs
                html += f"""
                            <tr>
                                <td>{s.id}</td>
                                <td>{data_str}</td>
    ...
    # ======================
    # FUNÇÃO PARA GERAR RELATÓRIO HTML DE ENTRADAS (PRODUTOS CADASTRADOS)
    # ======================
    def gerar_relatorio_entradas_html(produtos: List[ProdutoEstoque]) -> str:
    ...
    if st.session_state.almoxarifado_aba == 'ESTOQUE':
        ...
        with col_f2:
            categorias_lista = sorted(set([p.categoria for p in produtos if p.categoria]))
        ...
        if termo_busca:
            termo_lower = termo_busca.lower()
            produtos_filtrados = [p for p in produtos_filtrados if termo_lower in p.produto.lower()]
        
        if filtro_categoria != "(Todas)":
            produtos_filtrados = [p for p in produtos_filtrados if p.categoria == filtro_categoria]
        
        if filtro_status == "Estoque Baixo (<5)":
            produtos_filtrados = [p for p in produtos_filtrados if 0 < p.quantidade < 5]
        elif filtro_status == "Zerado":
            produtos_filtrados = [p for p in produtos_filtrados if p.quantidade <= 0]
        elif filtro_status == "Normal":
            produtos_filtrados = [p for p in produtos_filtrados if 5 <= p.quantidade <= 20]
        elif filtro_status == "Alto (>20)":
            produtos_filtrados = [p for p in produtos_filtrados if p.quantidade > 20]
        
        # KPIs
        total_produtos = len(produtos_filtrados)
        total_qtd = sum(p.quantidade for p in produtos_filtrados)
        qtd_zerado = len([p for p in produtos_filtrados if p.quantidade <= 0])
        qtd_baixo = len([p for p in produtos_filtrados if 0 < p.quantidade < 5])
    ...
    # ======================
        ...
        if st.session_state.almoxarifado_confirmar_saida:
            ...
            opcoes_produtos = [""] + sorted([p.produto for p in produtos if p.quantidade > 0])
            opcoes_categorias = sorted(set([p.categoria for p in produtos]))
            ...
            with st.form("form_saida"):
                ...
                with col1:
                    ...
                    if produto_selecionado:
                        for p in produtos:
                            if p.produto == produto_selecionado:
                                categoria_automatica = p.categoria
                                break
                ...
                with col2:
                    ...
                    if produto_selecionado:
                        for p in produtos:
                            if p.produto == produto_selecionado:
                                st.caption(f"📦 Estoque disponível: {p.quantidade:.2f} {p.ca if p.ca else ''}")
                                if quantidade > p.quantidade:
                                    st.warning(f"⚠️ A quantidade solicitada ({quantidade:.2f}) excede o estoque disponível ({p.quantidade:.2f})")
        ...
        with col_f1:
            filtro_produto_saida = st.selectbox(
                "🔎 Filtrar por Produto",
                options=["(Todos)"] + sorted(set([s.produto for s in saidas if s.produto])),
        ...
        if filtro_produto_saida != "(Todos)":
            saidas_filtradas = [s for s in saidas_filtradas if s.produto == filtro_produto_saida]
        if data_ini_saida:
            saidas_filtradas = [s for s in saidas_filtradas if s.data and s.data >= data_ini_saida]
        if data_fim_saida:
            saidas_filtradas = [s for s in saidas_filtradas if s.data and s.data <= data_fim_saida]
        
        # KPIs
        total_saidas = len(saidas_filtradas)
        total_qtd_saida = sum(s.quantidade for s in saidas_filtradas)
        ...
        if saidas_filtradas:
            ...
            for s in sorted(saidas_filtradas, key=lambda x: x.data if x.data else datetime.min, reverse=True):
                dados_saidas.append({
                    ...
                    "Quantidade": f"{s.quantidade:.2f}",
                    "Obs": s.obs[:30] + "..." if len(s.obs) > 30 else s.obs,
    ...
    # ======================
            ...
            with st.form("form_novo_produto"):
                ...
                with col1:
                    ...
                    categoria_nova = st.selectbox(
                        "Categoria*",
                        options=sorted(set([p.categoria for p in produtos if p.categoria])),
                        key="nova_categoria"
                    )
                ...
                if submitted_novo:
                    if not produto_nome:
                        ...
                        existe = any(p.produto.upper() == produto_nome.upper() for p in produtos)
                        if existe:
        ...
        with tab_editar:
            if produtos:
                opcoes_produtos_edit = sorted([p.produto for p in produtos])
                ...
                if st.session_state.almoxarifado_editando:
                    produto_selecionado_edit = st.selectbox(
                        ...
                        index=opcoes_produtos_edit.index(st.session_state.almoxarifado_editando.produto) if st.session_state.almoxarifado_editando.produto in opcoes_produtos_edit else 0
                    )
                else:
                    produto_selecionado_edit = st.selectbox(
                        "Selecione o produto para editar",
                ...
                if produto_selecionado_edit:
                    # Encontrar o produto
                    produto_edit = next((p for p in produtos if p.produto == produto_selecionado_edit), None)
                    
                    if produto_edit:
                        # Se mudou o produto selecionado, atualizar session state
                        if st.session_state.almoxarifado_editando is None or st.session_state.almoxarifado_editando.produto != produto_edit.produto:
                            st.session_state.almoxarifado_editando = produto_edit
                            st.rerun()
                        
                        with st.form("form_editar_produto"):
                            st.info(f"📌 Editando: {produto_edit.id} - {produto_edit.produto}")
                            
                            col1, col2 = st.columns(2)
                            
                            with col1:
                                st.text_input("ID", value=produto_edit.id, disabled=True, key="edit_id_display")
                                
                                categoria_edit = st.selectbox(
                                    "Categoria*",
                                    options=sorted(set([p.categoria for p in produtos if p.categoria])),
                                    index=sorted(set([p.categoria for p in produtos if p.categoria])).index(produto_edit.categoria) if produto_edit.categoria in sorted(set([p.categoria for p in produtos if p.categoria])) else 0,
                                    key="edit_categoria"
                                )
                                
                                produto_nome_edit = st.text_input(
                                    "Produto*",
                                    value=produto_edit.produto,
                            ...
                            if submitted_edit:
                                if not produto_nome_edit:
                                    ...
                                    produto_atualizado = ProdutoEstoque(
                                        id=produto_edit.id,
        ...
        with tab_excluir:
            if produtos:
                opcoes_produtos_excluir = sorted([p.produto for p in produtos])
                ...
                if produto_selecionado_excluir:
                    produto_excluir = next((p for p in produtos if p.produto == produto_selecionado_excluir), None)
                    
                    if produto_excluir:
                        st.warning(f"⚠️ Você está prestes a excluir o produto: **{produto_excluir.produto}** (ID: {produto_excluir.id})")
                        st.caption(f"Quantidade em estoque: {produto_excluir.quantidade:.2f}")
                        
                        if produto_excluir.quantidade > 0:
                            st.warning(f"⚠️ Este produto tem {produto_excluir.quantidade:.2f} unidades em estoque. A exclusão removerá o cadastro, mas o histórico de saídas permanecerá.")
                        
                        confirmar_excluir = st.checkbox(f"✅ Confirmo que desejo EXCLUIR permanentemente o produto {produto_excluir.produto}")
                        
                        if confirmar_excluir:
                            if st.button("🗑️ CONFIRMAR EXCLUSÃO", type="primary", use_container_width=True):
                                sucesso, msg = excluir_produto(produto_excluir.id)
                                if sucesso:
                                    st.success(msg)
                                    st.balloons()
                                    st.rerun()
