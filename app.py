...
with st.sidebar:
    ...
    # Criar lista plana de todos os itens
    TODOS_ITENS = []
    ...
    # CSS para estilizar a sidebar com categorias
    st.markdown(f"""
    <style>
    /* Estilo para os cabeçalhos das categorias */
    .sidebar-categoria {{
        font-family: 'JetBrains Mono', monospace;
        font-size: 10px;
        font-weight: 700;
    ...
    .sidebar-categoria .icone {{
        font-size: 14px;
    }}
    /* Esconder o label do radio e estilizar os itens */
    .stRadio > label {{
    ...
    # Renderizar cabeçalhos das categorias e o radio
    for categoria, itens in CATEGORIAS.items():
        # Verificar se algum item da categoria está ativo
        is_active = any(item == aba_atual for item in itens)
        cor = THEME['accent_cyan'] if is_active else THEME['text_muted']
        
        st.markdown(f"""
        <div class="sidebar-categoria" style="color: {cor};">
            <span class="icone">{categoria.split()[0]}</span> {categoria.split()[1]}
        </div>
        """, unsafe_allow_html=True)
        
        # Radio com os itens desta categoria (apenas os itens da categoria atual)
        # Usamos um radio separado para cada categoria, mas apenas um é visível
        # Na verdade, vamos usar um único radio com todos os itens,
        # mas agrupar visualmente por categoria
        for item in itens:
            # Criar uma opção com prefixo da categoria para manter o agrupamento
            pass

    # Radio único com todos os itens (funcional)
    # Personalizar a exibição com ícones
    ICONES = {
        "PRENSADOS": "🔩",
        "SOPRO": "💨",
        "TÊMPERA": "🔥",
    ...
    # Criar opções com ícones para exibição
    opcoes_com_icones = []
    for item in TODOS_ITENS:
        icone = ICONES.get(item, "•")
        opcoes_com_icones.append(f"{icone} {item}")

    # Radio principal
    selected_index = TODOS_ITENS.index(aba_atual) if aba_atual in TODOS_ITENS else 0
    selected_com_icone = st.radio(
        "Navegação",
        options=opcoes_com_icones,
        index=selected_index,
        label_visibility="collapsed",
        key="nav_radio_tree"
    )
    
    # Extrair o nome do item selecionado (remover o ícone)
    selected_item = selected_com_icone.split(" ", 1)[1] if " " in selected_com_icone else selected_com_icone
    st.session_state.aba_selecionada = selected_item
