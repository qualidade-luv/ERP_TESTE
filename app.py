...
# ==================================================================================================
# PRENSADOS - VERSÃO COM SUPABASE (PRIORIDADE ABSOLUTA) E FALLBACK GOOGLE SHEETS
# ==================================================================================================
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if dados:
                    ...
                    # Adicionar coluna ANO_MES se não existir
                    if 'DATA' in df.columns:
                        df['ANO_MES'] = df['DATA'].dt.to_period('M').astype(str)
                        df['DIA_SEMANA'] = df['DATA'].dt.day_name()
                        df['SEMANA'] = df['DATA'].dt.isocalendar().week
    ...
    with st.spinner("🔄 Carregando dados de prensados..."):
    ...
    # Mostrar resultado
    if not df_base.empty:
        fonte = "Supabase" if supabase_ok and not carregar_dados_prensados_supabase().empty else "Google Sheets (fallback)"
        if fonte == "Supabase":
    ...
    # TODO O RESTO DO CÓDIGO DO MÓDULO PRENSADOS CONTINUA IGUAL
    # (O código a partir daqui é exatamente o mesmo do arquivo original)
